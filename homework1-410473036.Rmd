...
library(lubridate)
hour(twTime)
nightOwl <- ((1 <= hour(twTime))& (hour(twTime) <= 6))
nightOwl
```
...
```{r ans31}
earnedCredit <- ((grade < 60) == 0)*credit
earnedCredit
```
...
```{r ans34}
pass <- (studentID == "046S6f645") & (schoolYear == "100") & (credit >= "0")
gpa100ChosenStudent<-sum(pass*credit*grade)/
